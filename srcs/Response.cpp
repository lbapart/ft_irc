...
const std::string&	OKleaveSuccess(const std::string& nickname, const std::string& channel)
{
	std::string response = ":" + nickname + "!" + std::string(SERVER_NAME) + "@localhost PART " + channel + "\r\n";
	return response;
}

const std::string&	ERRleaveFailed(const std::string& nickname, const std::string& channel)
{
